...
//ADDED BY ARKDE - teachers of course
$teachers = get_field('teachers',$parent_course->ID);
	...
	<div class="lms-topic-sidebar-data">
        ...
        <div class="lms-topic-sidebar-course-navigation">
            <div class="ld-course-navigation">
                <?php do_action( 'learndash-underdev', $course_id, $user_id,"small" ); ?>
                ...
                <h2 class="course-entry-title"><?php echo $parent_course_title; ?></h2>
                 <div class="teacher-list has-tiny-padding-top">
                    <?php foreach( $teachers as $teacher ) { ?>
                            <div class="teacher-item tiny">
                                <?php echo get_the_post_thumbnail($teacher->ID,'tiny'); ?>
                                <div class="content">
                                    <p class="is-marginless"><?php echo $teacher->post_title ?></p>
                                </div>
                            </div>
                    <?php } ?>
					...
					<?php foreach( $lession_list as $lesson ) { ?>
						...
						<li class="lms-lesson-item <?php echo $lesson->ID == $post->ID ? 'current' : 'lms-lesson-turnover'; ?> <?php echo $lesson_sample . ' ' . $locked_class; ?> <?php echo !empty( $lesson_topics ) ? '' : 'bb-lesson-item-no-topics'; ?>">
                            ...
                            <?php if( ! empty( $lesson_topics ) ) : ?>
                                <span class="lms-toggle-lesson"><i class="bb-icons bb-icon-chevron-down is-medium has-text-weight-bold"></i></span>
						    <?php endif; ?>
							<a href="<?php echo get_permalink( $lesson->ID ); ?>" title="<?php echo $lesson->post_title; ?>" class="bb-lesson-head flex">
                                ...
                                <?php
                                if( $lesson_progress['percentage'] == '100' && $completed) {
                                ...
                                <div class="flex align-items-center <?php echo ($lesson_progress['percentage'] == '100' && $completed) ? 'bb-check-completed' : 'bb-check-not-completed'; ?>">
									<div class="bb-lms-progress-wrap" data-balloon-pos="left" data-balloon="<?php echo $lesson_progress_data_balloon; ?>">
                                        <?php if ($lesson_progress['percentage'] == '100' && $completed) { ?>
                                                       ...
                                                       title="<?php echo $lesson_topic->post_title; ?>">
														...
														if ( ! empty( $lesson_video_enabled ) ) { ?>
                                                            <span class="bb-lms-ico bb-lms-ico-topic"><i
                                                                        class="bb-icons bb-icon-play-circle-fill"></i></span>
														<?php } else if (learndash_lesson_hasassignments($lesson_topic)) { ?>
                                                             <span class="bb-lms-ico bb-lms-ico-topic"><i
                                                                        class="bb-icons bb-icon-feather"></i></span>
                                                            <?php } else { 
                                                                //check if it has a certificate category
                                                                if(has_term('certificado', 'ld_topic_category',$lesson_topic))
                                                                { ?>
                                                                    <span class="bb-lms-ico bb-lms-ico-topic"><i
                                                                        class="bb-icons bb-icon-badge"></i></span>
                                                        <?php   }
                                                                else{ ?>
                                                                    <span class="bb-lms-ico bb-lms-ico-topic"><i
