...
<div class="ld-table-list-item <?php echo $topic_video_progression_class; ?>" id="<?php echo esc_attr('ld-table-list-item-' . $topic->ID ); ?>">
    <a class="<?php echo esc_attr($topic_class); ?>" href="<?php echo esc_attr( learndash_get_step_permalink( $topic->ID, $course_id ) ); ?>">
        ...
        $duration = get_field("duration",$topic->ID);
        if($duration)
        {
            echo "<span class='lesson-duration'>".$duration."</span>";
        }
        ?>
        ...
        <span class="ld-topic-title <?php echo learndash_lesson_hasassignments($topic) ? 'assignment' : '' ?>"><?php echo wp_kses_post($topic->post_title); ?></span>
        <?php
        /**
